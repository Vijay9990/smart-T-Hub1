...
print("Flask app has been loaded")
app = Flask(__name__)
app.secret_key = 'ride_sharing'
mongo_url = os.environ.get('MONGO_URI')
my_client = pymongo.MongoClient(mongo_url)
my_database = my_client["ride_sharing"]
rider_collection = my_database['rider']
driver_collection = my_database['driver']
location_collection = my_database['location']
rides_collection = my_database['rides']
...
APP_ROOT_VEHICLE = APP_ROOT + "/static/vehicle"

APP_ROOT = os.path.dirname(os.path.abspath(__file__))
APP_ROOT_PASSENGER = APP_ROOT + "/static/passenger"
...
def post_ride_action():
    try:
        ...
        if not driver_id:
            return render_template("message_action.html", message="Driver not logged in.")

        ride_date = datetime.combine(start_date_time.date(), datetime.min.time())  # convert to datetime

        new_ride = {
            "ride_type": ride_type,
            "ride_start_location_id": ObjectId(ride_start_location_id),
            "ride_end_location_id": ObjectId(ride_end_location_id),
        ...
        print("Inserted ride with ID:", result.inserted_id)
        return render_template("message_action.html", message="Data inserted successfully")

    except Exception as e:
        print("Exception occurred:", e)
        print(traceback.format_exc())
        return render_template("message_action.html", message="An error occurred: " + str(e))
...
def for_got_password_rider_action2():
    ...
    return render_template("message.html", message="Password Reset Successfully")
#added this to run app in localmachine
